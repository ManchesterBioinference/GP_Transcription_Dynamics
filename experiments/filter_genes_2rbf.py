...
1) Fit individual GPs for mRNA and prem_RNA with RBF kernel where l->inf;
...
import sys
sys.path.append('..')
from utils.utilities import fit_rbf, fit_noise_rbf,fit_noise, init_hyperparameters
import matplotlib.pyplot as plt
    ...
    while count_Cholesky_fail < 20:
        try:
            if fail:
                init_lengthscale, init_variance = init_hyperparameters(t0)
                fail = False
            m, lik = fit_noise_rbf(data_full, init_variance)
...
df.to_csv('../output/filtered_genes_zygotic_tr_95_genes_2rbf.csv',index=False)
