...
class Gaussian_vgp(ScalarLikelihood):
    r"""
    ...
    def __init__(self, variance_m=1.0, variance_p=1.0,variance_lower_bound=DEFAULT_VARIANCE_LOWER_BOUND, **kwargs):
        """
        ...
        if variance_m <= variance_lower_bound:
            raise ValueError(
                f"The variance of the Gaussian likelihood must be strictly greater than {variance_lower_bound}"
            )

        self.variance_m = gpflow.Parameter(variance_m, transform=gpflow.utilities.positive(lower=variance_lower_bound))
        self.variance_p = gpflow.Parameter(variance_p, transform=gpflow.utilities.positive(lower=variance_lower_bound))
    ...
    def _predict_log_density(self, Fmu, Fvar, Y):
        return tf.reduce_sum(logdensities.gaussian(Y, Fmu, Fvar + self.variance_m), axis=-1)
